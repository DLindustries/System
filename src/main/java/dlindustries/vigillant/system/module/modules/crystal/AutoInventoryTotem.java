...
import dlindustries.vigillant.system.utils.FakeInvScreen; //  ADDED: for invisible inventory screen
...
public final class AutoInventoryTotem extends Module implements TickListener {
    ...
    public AutoInventoryTotem() {
        super(EncryptedString.of("Autototem-blatant"),
    ...
    @Override
    public void onTick() {
        //  ADDED: Use FakeInvScreen to open invisible inventory screen
        if (shouldOpenScreen() && autoOpen.getValue())
            mc.setScreen(new FakeInvScreen(mc.player)); // ADDED
